...
function HostCard({ host }: { host: Host }) {
  ...
  return <Link to={`/hosts/${encodeURIComponent(host.publicId)}?range=1h`} className="group relative block overflow-hidden rounded-md border border-line bg-surface p-4 transition-colors hover:border-line-strong">
    ...
    <div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="truncate text-sm font-semibold">{host.nickname}</h2><p className="mt-1 truncate text-xs text-muted">{host.hostname || "尚未上报系统信息"} · {host.platform || host.os || "未知系统"} {host.platformVersion}</p></div><StatusBadge online={host.online} /></div>
